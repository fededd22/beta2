...
from keepalive import DATA_DIR, add_log, start_web_server, keep_alive_task, get_status, get_public_domain
...
def _ensure_user_site_on_path():
    """Rend visibles les paquets installés avec `pip --user` après le démarrage du bot."""
    try:
    ...
    for lib in libs:
        ...
        try:
            proc = await asyncio.create_subprocess_exec(
                *PIP_CMD, "install", "--no-input", "--progress-bar", "off", lib,
